...
export default function MoneyFlowList({ stocks }: MoneyFlowListProps) {
  // 按主力净流入排序
  const sorted = [...stocks].sort((a, b) => (b.mainInflow ?? 0) - (a.mainInflow ?? 0))
  ...
  return (
    <div className="glass rounded-xl p-5 card-hover">
      <div className="flex items-center gap-2 mb-4">
        ...
        <h2 className="text-lg font-semibold text-white">主力资金流向</h2>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody>
            {display.length === 0 ? (
              <tr><td colSpan={5} className="text-center py-4 text-gray-500">暂无数据</td></tr>
            ) : (
              display.map((s) => (
                <tr key={s.code} className="border-b border-white/5 hover:bg-white/5">
                  ...
                  <td className={`text-right font-medium ${(s.mainInflow ?? 0) >= 0 ? 'text-red-400' : 'text-green-400'}`}>
                    {fmt(s.mainInflow)}
                  </td>
                  <td className={`text-right ${(s.hugeInflow ?? 0) >= 0 ? 'text-red-400' : 'text-green-400'}`}>
                    {fmt(s.hugeInflow)}
                  </td>
                </tr>
              ))

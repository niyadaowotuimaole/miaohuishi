...
interface IndexData { name: string; code: string; price: number; change: number; changePercent: number }
interface Props { indices?: IndexData[] | null }
const demo: IndexData[] = [
...
export default function StockData({ indices }: Props) {
  const data = indices || demo
  return (
    <div className="glass rounded-xl p-5 card-hover">
      <div className="flex items-center gap-2 mb-4">
        <Flame className="w-5 h-5 text-orange-400" />
        <h2 className="text-lg font-semibold text-white">大盘指数</h2>
        {indices && <span className="ml-2 px-2 py-0.5 bg-green-500/20 text-green-400 rounded text-xs">实时</span>}
      </div>
      <div className="grid grid-cols-2 gap-3">
        {data.map((index) => (
          <div key={index.code} className="p-3 bg-white/5 rounded-lg">
